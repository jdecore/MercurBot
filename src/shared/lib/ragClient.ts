...
import type { PdfChunk } from '../../entities/pdf/extractors/pdf'
...
class RagClient {
  ...
  private initWorker() {
    ...
    try {
      this.worker = new Worker(
        new URL('../../entities/rag/worker.ts', import.meta.url),
        { type: 'module' }
      )
  ...
  /**
   * Trigger embeddings model pre-download in the worker.
   * The worker lazy-loads @huggingface/transformers on INDEX_DOCUMENT.
   * This sends a PREWARM action so the model downloads in background
   * before the user uploads their first PDF.
   */
  public prewarm() {
    if (this.worker) {
      this.worker.postMessage({ action: 'PREWARM' })
    }
  }
