...
3. Pregunta en lenguaje natural → respuesta en streaming con citas `[Pág. N]` clicables: abren el visor embebido en esa página. Al abrir, la IA te recibe con un briefing de 3 puntos clicables (generado una vez, sin bloquear). El shell tiene sidebar con `+ Nuevo análisis`, Recientes (biblioteca OPFS), Capacidades y usuario local — drawer en mobile; la top-bar muestra el estado honesto del motor (`100% local` / `pensando…` / `híbrida ✦ modelo`, solo lectura)
