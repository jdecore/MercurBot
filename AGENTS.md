...
- **Fase 28 — Test live + fixes honestidad RAG (2026-09-12):** test E2E contra `https://copixi.vercel.app/`: (1) `/api/chat` OK — el LLM responde (200, SSE válido); (2) **CSP live desactualizada** — el deploy sirve `connect-src` sin huggingface/jsdelivr (el fix de Fase 27 está en `vercel.json` local sin desplegar): en producción el modelo MiniLM no puede descargarse y todo cae a `lexical_only` silencioso → **requiere redeploy (push → Vercel)**; (3) fixes: `ragClient.didLastSearchUseFallback()` — el watchdog marcaba fallback sin reflejarlo y la UI mentía "búsqueda combinada" (`ragPipeline` ahora usa el flag real de la búsqueda); `api/chat buildContextBlock` inyecta `searchMode` al prompt (el LLM sabe si los fragmentos son combinados o solo literales y puede ser transparente). Verificado: `pnpm build` ok, `pnpm lint` solo warnings preexistentes.

---

*Última actualización: 2026-09-12 (Fase 28 Test live + fixes honestidad RAG — CSP live desactualizada pendiente de redeploy, watchdog honesto, searchMode al prompt; build ok) — Contrato vivo de Copixi. Cualquier desviación debe justificarse según §41.*
