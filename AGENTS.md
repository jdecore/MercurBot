...
- **Fase 42 — Tarjeta de personalización del robot (2026-09-14):** el botón "Personalizar robot" abre una tarjeta modal (Radix Dialog) en vez del apilado inline: header con título + descripción + cerrar, héroe con preview vivo y nombre, y secciones (Cara / Mi robot se llama + Sorpréndeme / Diseño 19 opciones / Ojos / Extra 8 / Unidad base 6). 🎲 reemplazado por icono reload (Pixelarticons §18). Estilos token-driven en `Mascota.css` (overlay, card responsive, secciones con divisores). Sin cambios de lógica ni persistencia. Verificado: `pnpm build` ok, `pnpm lint` 6 warnings preexistentes.
...
- **Fase 43 — Paleta "Gris + Maracuyá + Hoja" (2026-09-17):** a pedido del usuario se abandona el beige/mango terroso: base gris neutra (`--color-bg #E7E7E3`), primario a gris petróleo (`#2F3A3C`, deja el protagonismo al acento), acento redefinido a **amarillo maracuyá** (`#E3A008` claro / `#F5B301` oscuro, `--color-on-accent` tinta `#1C1A10` — blanco sobre maracuyá falla AA y queda prohibido) y nuevo **`--color-tropical` verde hoja** (`#0E7C6B`/`#3DD598`) para éxito/progreso/píldora RAG; `--color-success` apunta al tropical. Barrido: `mascot-switch-btn.active` a `on-accent`, scrollbar a `var(--color-border)`, grano SVG a gris neutro; `App.css` sin hex fuera de tokens (solo scrims rgba neutros), bloque oscuro intacto en tokens. QA: contraste 12/13 real pass (el FAIL es blanco-sobre-maracuyá, combinación prohibida por diseño), 0 Tailwind/shadcn/lucide/`VITE_`, `pnpm build` ok, `pnpm lint` sin warnings nuevos. `Mascota.css` sigue excepción de personaje. Disciplina del 10% heredada de Fase 38: maracuyá solo donde actúa.

- **Fase 44 — Shell QwenWork-honesto (2026-09-17):** adaptación de la referencia QwenWork sin copiar su backend. (A) `src/components/layout/Sidebar.tsx` (nuevo): `+ Nuevo análisis` (limpia doc/briefing/panel sin borrar biblioteca), `Recientes` como lista visible (reutiliza `listLibrary()`, empty state honesto, quitar ✕), `Capacidades` locales estáticas (resumir, citas, buscar, graficar) + nota de privacidad; footer con usuario local + `Personalizar` + `¿Cómo funciona?`. `src/App.tsx`: shell grid `264px + main`, top-bar adelgazada (hamburger solo <900px, brand, pill `Leyendo:`, `EngineStatus`, `Cargar PDF`); drawer Radix con el mismo Sidebar en mobile; `DocLibrary.tsx` (select) eliminado — el archivo muere, la lógica OPFS vive en `lib/docLibrary.ts`. Sin Skills/Connectors/Drive/Scheduled (implican backend §5.4/§31) ni créditos falsos. (B) Héroe: saludo con nombres bajo la mascota (180px intacta), starters sin doc (`Subir mi PDF`) y con doc (3 chips que rellenan el input, sin auto-envío), placeholder dinámico con nombre del PDF (`truncateName`), dock `sticky bottom`. (C) `src/components/layout/EngineStatus.tsx` (nuevo): `100% local` sin doc, `pensando…` en streaming, `híbrida/literal ✦ modelo` tras responder — solo lectura vía evento `copixi:engine-status` (el fallback lo decide `/api/chat`). Iconos `plus/menu/chart/lock` añadidos al mapa `Icon.tsx` (Pixelarticons §18 intacto). Verificado: `pnpm build` ok, `pnpm lint` 7 warnings preexistentes (0 nuevos), 0 Tailwind/shadcn/lucide/`VITE_`, sin nuevas deps (§34), sin backend (§11), `prefers-reduced-motion` en el pulso.

- **Fix mic + diagnóstico live (2026-09-17):** el dictado no funcionaba en producción porque el propio `vercel.json` enviaba `Permissions-Policy: microphone=()` (bloquea el mic a toda la app) → cambiado a `microphone=(self)`. Los errores live `502` + CSS `MIME text/plain` eran página vieja en caché del navegador (el HTML cacheado pedía `index.CenbCCxk.css` de un deploy anterior, ya podado → 404 text/plain bloqueado por `nosniff` propio): verificado que live sirve el HTML actual (`index.Dx8t6-Bh.css` 200 text/css) y `/api/chat` responde 405/400 según método (función viva). Requiere redeploy (push) para el header + recarga dura (Ctrl+Shift+R) en el navegador afectado.

- **Sin top-bar + fix aria-hidden + 502 live (2026-09-17):** a pedido del usuario se elimina la `canvas-top-bar` (brand duplicado + pill `Leyendo:`): `Cargar PDF` y `EngineStatus` viven ahora en el sidebar (desktop y drawer); en mobile solo queda un botón flotante `fab-menu` (hamburger) + el adjuntar del dock. CSS muerto eliminado (`canvas-top-bar/brand/actions/dataset-pill/sidebar-toggle`). Fix del warning `Blocked aria-hidden`: al abrir doc/personalizar/cómo-funciona desde el drawer se hace `blur()` del foco antes de cerrar, para que Radix pueda ocultar el fondo. Sobre el `POST /api/chat 502`: probado en live con payload real → **200 vía OpenRouter** (Gemini/Groq caídos o sin key válida en ese momento, el fallback funcionó); el 502 JSON de `generate()` es el camino diseñado cuando todos los proveedores fallan (transitorio), no un crash — `buildContextBlock` es defensivo y no lanza. Los warnings `Permissions-Policy: run-ad-auction/join-ad-interest-group/...` NO vienen de nuestro header (solo declara `geolocation/microphone/camera`) sino de Chrome/extensiones; se ignoran.

- **Fase 45 — Sidebar contraíble estilo Copilot (2026-09-18):** el menú tiene dos anchos con el mismo contenido (nada nuevo §41: sin Tareas/Proyectos/Imaginar falsos, sin paywall). `Preferences += sidebarCollapsed` (migra prefs viejas, default `false`); `App.tsx` levanta `railCollapsed` + `toggleRail` con persistencia y aplica `.rail` al shell y al aside. `Icon.tsx` += `chevron-left/right` (Pixelarticons §18). `Sidebar.tsx` rama `collapsed`: rail 64px con `EngineStatus compact` (solo punto + tooltip), iconos Nuevo/Cargar/Recientes (el de recientes expande, no navega a ciegas), usuario/personalizar y toggle `chevron-right` al pie; expandido añade botón `Contraer` con `chevron-left` al pie + `aria-expanded`. CSS: `.app-shell.rail` 64px con transición 200ms (anulada en `prefers-reduced-motion`); drawer mobile intacto (siempre completo). Verificado: `pnpm build` ok, `pnpm lint` 7 warnings preexistentes, dev 200, sin nuevas deps (§34).
...
- **Auditoría del agente externo + accent-ink + robot compacto (2026-09-18):** un agente afirmó 5 problemas; verificados contra código: (1) citas estáticas → FALSO, son botones con salto+resaltado desde Fases 18/30/31; (2) robot decorativo → parcial, se compacta pero se conserva; (3) CAPACIDADES ruido → CIERTO, se muda; (4) split fijo → cierto pero fuera de alcance a pedido; (5) paleta bajo contraste → parcial: botones y textos pasaban, pero el maracuyá como primer plano sobre claro fallaba (1.82–2.38, confirmado por medición — tenía razón el usuario). Fix: nuevo `--color-accent-ink #7C5600` (5.31 gris / 6.59 blanco; en oscuro apunta al accent brillante 9.63) + regla `accent`=rellenos con tinta encima / `accent-ink`=texto, bordes, focos, puntos; re-apuntados hint/focus/chip-hover/engine-busy/hear-border. Robot: `size 180→72` en fila con doc (`mascot-stage.compact`, saludo corto con hint de relectura, personalizar oculto del hero), clic re-lee última respuesta (evento `copixi:reread`, sin cuota), puente `copixi:mascota-mood` en App (los dispatches del chat no llegaban al robot), `dormido` tras 5 min inactivo. CAPACIDADES sale del sidebar (queda Recientes+acciones+privacidad) y vive como 4 chips accionables con doc sin mensajes. Verificado: `pnpm build` ok, `pnpm lint` 7 warnings preexistentes, contraste 7/7 pass, sin nuevas deps (§34).
